...
// POST /api/resume/bullets/optimize-text - Optimize raw bullet text with OpenAI
export const POST = async (request: NextRequest) => {
  try {
    ...
    const { bulletContent, company, role } = body;

    if (!bulletContent || typeof bulletContent !== 'string' || bulletContent.trim().length === 0) {
      return NextResponse.json(
        { error: 'Bullet content is required' },
        { status: 400 }
      );
    }
    ...
    const prompt = createOptimizationPrompt(bulletContent, company, role);
