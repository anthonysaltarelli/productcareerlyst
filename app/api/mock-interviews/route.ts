...
export async function GET() {
  try {
    ...
    const { data: interviews, error: fetchError } = await supabase
      ...
      .select(
        'id, status, started_at, ended_at, duration_seconds, call_quality_rating, self_performance_rating, ai_evaluation, created_at, interview_mode, question_id, job_context, adhoc_question'
      )
      .eq('user_id', user.id)
