...
export const GoalsAndChallengesStep = ({ onNext, onBack }: GoalsAndChallengesStepProps) => {
  ...
  const handleTargetRoleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (!filteredTargetRoleSuggestions.length) return;
    ...
    if (event.key === 'ArrowDown') {
      ...
      setHighlightedTargetRoleIndex((prevIndex) => {
        ...
        return prevIndex + 1 >= filteredTargetRoleSuggestions.length ? 0 : prevIndex + 1;
      });
      return;
    }
    ...
    if (event.key === 'ArrowUp') {
      ...
      setIsTargetRoleDropdownOpen(true);
      setHighlightedTargetRoleIndex((prevIndex) => {
        if (prevIndex === null) return filteredTargetRoleSuggestions.length - 1;
        return prevIndex - 1 < 0 ? filteredTargetRoleSuggestions.length - 1 : prevIndex - 1;
      });
      return;
    }
    ...
    if (event.key === 'Enter' && highlightedTargetRoleIndex !== null) {
      ...
      if (selectedRole) handleSelectTargetRole(selectedRole);
      return;
    }
  ...
  const handleContinue = useCallback(async () => {
    ...
    try {
      ...
      // Save to database
      await updateStep('goals', stepData);

      // Track Amplitude event
      trackEvent('Onboarding Step Completed', {
        'Step': 'goals',
        'Step Name': 'Goals & Challenges',
      ...
      <div className="space-y-6 md:space-y-8">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          <input
            ...
            onFocus={() => setIsTargetRoleDropdownOpen(true)}
