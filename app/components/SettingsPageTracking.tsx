...
interface SettingsPageTrackingProps {
  stats: DashboardStats | null;
  subscription: Subscription | null;
  userCreatedAt?: string | null;
  initialActiveTab: 'profile' | 'account' | 'logout';
  profileCompletion: {
...
/**
 * Component to track settings page views with comprehensive user state context
 * Uses a ref to prevent duplicate tracking in React StrictMode
 */
export const SettingsPageTracking = ({
  stats,
  subscription,
  userCreatedAt,
  initialActiveTab,
  profileCompletion,
}: SettingsPageTrackingProps) => {
  ...
  useEffect(() => {
    ...
    // Track settings page view with all context
    trackEvent('User Viewed Settings Page', {
      ...
      'Has First Name': profileCompletion.hasFirstName,
      'Has Last Name': profileCompletion.hasLastName,
      'Has LinkedIn URL': profileCompletion.hasLinkedIn,
      'Has Portfolio URL': profileCompletion.hasPortfolio,
      'Profile Fields Filled Count': profileCompletion.fieldsFilledCount,

      // User state context (comprehensive)
      ...userStateContext,
    });
  }, [stats, subscription, coach, compensation, impactPortfolio, careerTracker, userCreatedAt, initialActiveTab, profileCompletion]);
