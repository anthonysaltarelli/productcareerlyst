...
export default function InterviewDetailPage() {
  ...
  const [isMarkingThankYouSent, setIsMarkingThankYouSent] = useState(false);
  ...
  const handleMarkThankYouSent = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to mark thank you as sent');
      }

      await refetch();
    } catch (err) {
      setSubmitError(err instanceof Error ? err.message : 'Failed to mark thank you as sent');
    } finally {
      setIsMarkingThankYouSent(false);
    }
  };
    ...
    <div className="min-h-screen p-8 md:p-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className={`p-6 rounded-[2rem] shadow-[0_8px_0_0_rgba(0,0,0,0.1)] border-2 ${
            interview.thank_you_sent_at
              ? 'bg-gradient-to-br from-green-100 to-emerald-100 border-green-300'
              : 'bg-white border-gray-300'
          }`}>
            <h3 className="text-xl font-black text-gray-900 mb-3">Thank You Note</h3>
            {interview.thank_you_sent_at ? (
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-green-700 font-bold">Sent</p>
                  <p className="text-green-600 text-sm font-medium">
                    {new Date(interview.thank_you_sent_at).toLocaleDateString('en-US', {
                      month: 'short',
                      day: 'numeric',
                      year: 'numeric',
                      hour: 'numeric',
              ...
              <button
                ...
                disabled={isMarkingThankYouSent}
                className="w-full px-4 py-3 rounded-[1rem] bg-gradient-to-br from-green-500 to-emerald-500 shadow-[0_4px_0_0_rgba(16,185,129,0.4)] border-2 border-green-600 hover:translate-y-0.5 hover:shadow-[0_2px_0_0_rgba(16,185,129,0.4)] font-black text-white text-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 flex items-center justify-center gap-2"
              >
                {isMarkingThankYouSent ? (
                  <>
