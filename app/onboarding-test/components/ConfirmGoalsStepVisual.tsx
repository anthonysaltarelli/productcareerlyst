...
// Weekly goal type from PlanDisplayStepVisual
...
interface ConfirmGoalsStepVisualProps {
  onNext: () => void;
  onBack: () => void;
...
const WEEKLY_GOAL_INFO: Record<string, { displayName: string; description: string; unit: string; min: number; max: number }> = {
  'weekly-applications': { displayName: 'Job Applications', description: 'Apply to quality PM roles', unit: 'per week', min: 1, max: 30 },
  'weekly-networking-calls': { displayName: 'Networking Calls', description: 'Schedule calls with PMs', unit: 'per week', min: 1, max: 10 },
  'weekly-outreach-emails': { displayName: 'Outreach Emails', description: 'Send personalized emails', unit: 'per week', min: 1, max: 50 },
...
export const ConfirmGoalsStepVisual = ({ onNext, onBack, weeklyGoals }: ConfirmGoalsStepVisualProps) => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-8">
      <div className="mb-6 md:mb-8">
        ...
        <p className="text-base md:text-lg text-gray-700 font-semibold">
          Here are the weekly targets we recommend based on your plan. Adjust the numbers to fit your schedule.
        </p>
      </div>
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          max={max}
                          value={value}
                          onChange={(e) => handleGoalValueChange(goal.id, parseInt(e.target.value, 10) || min)}
      ...
      {enabledGoalsCount > 0 && (
        <div className="mt-6 p-4 bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 rounded-xl">
          <p className="text-sm font-semibold text-purple-800">
            ✅ You have {enabledGoalsCount} weekly goal{enabledGoalsCount !== 1 ? 's' : ''} selected. We'll track your progress and help you stay on track!
          </p>
        </div>
      )}
      ...
      <div className="mt-8 flex flex-col gap-3 sm:flex-row-reverse sm:items-center sm:justify-between">
        <button
          onClick={onNext}
          className="w-full sm:w-auto px-6 md:px-8 py-3 bg-gradient-to-br from-purple-500 to-pink-500 text-white font-black rounded-xl hover:from-purple-600 hover:to-pink-600 transition-all text-sm md:text-base"
        >
          Confirm & Continue →
        </button>
        <button
